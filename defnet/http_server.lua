...
function M.create(port)
	...
	-- chunked 
	instance.css = {
		header = function(css, status)
			local headers = {
				"HTTP/1.1 " .. (status or M.OK),
				instance.server_header,
				"Content-Type: text/css",
				json and ("Content-Length: " .. tostring(#css)) or "Transfer-Encoding: chunked",
			}
			if instance.access_control then
				headers[#headers + 1] = "Access-Control-Allow-Origin: " .. instance.access_control
			end
		...
		response = function(css, status)
			return instance.css.header(css, status) .. (css or "")
		end
	}
	setmetatable(instance.css, { __call = function(_, css, status) return instance.css.response(css, status) end })
	...
	-- chunked 
	instance.script = {
		header = function(script, status)
			local headers = {
				"HTTP/1.1 " .. (status or M.OK),
				instance.server_header,
				"Content-Type: text/javascript",
				json and ("Content-Length: " .. tostring(#script)) or "Transfer-Encoding: chunked",
			}
			if instance.access_control then
				headers[#headers + 1] = "Access-Control-Allow-Origin: " .. instance.access_control
			end
		...
		response = function(script, status)
			return instance.script.header(script, status) .. (script or "")
		end
	}
	setmetatable(instance.script, { __call = function(_, script, status) return instance.script.response(script, status) end })
